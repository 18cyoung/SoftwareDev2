...
Contact lists are formatted as a list containing 2 sublists, the first one for names, the second for dates of birth
formatted as follows [[firstname, lastname],[day, month, year]]
"""
...
def ChangeBirthday(cb_contactList):
    ...
    ydate = input("Year: ")
    if fname != "":
        cb_contactList[index][0][0] = fname
    if sname != "":
        cb_contactList[index][0][1] = sname
    if ddate != "":
        cb_contactList[index][1][0] = ddate
    if mdate != "":
        cb_contactList[index][1][1] = mdate
    if ydate != "":
        cb_contactList[index][1][2] = ydate
...
def LookupBirthday(lb_contactlist):
    ...
    print(lb_contactlist[index][0][0], lb_contactlist[index][0][1],"has their birthday on ", lb_contactlist[index][1][0], "/",lb_contactlist[index][1][1] ,"/",lb_contactlist[index][1][2] )
    return
